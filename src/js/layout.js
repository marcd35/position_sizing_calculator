(function initLayout() {
  ...
  function insertHeader() {
    ...
    var calculatorLinks = NAV_LINKS.slice(0, 3)
      .map(function (link) {
    ...
    var themeSwitcherHtml =
      ...
      '    aria-checked="' +
      (currentTheme === 'grayscale' ? 'true' : 'false') +
      '" ' +
    ...
    var headerHtml =
      ...
      '      <a class="site-nav__link instructions-link desktop-only" href="' +
      escapeText(instructionsLink.href) +
      '"' +
      (isInstructionsCurrent ? ' aria-current="page"' : '') +
      '>' +
      escapeText(instructionsLink.label) +
      '</a>' +
      '      <div class="theme-switcher-desktop desktop-only">' +
      themeSwitcherHtml +
      '</div>' +
